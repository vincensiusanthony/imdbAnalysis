      ...
      <ChartCard title="Actor Genre Specialization" icon="🎭" heightClass="h-96" iconBgClass="bg-pink-500/20" iconTextClass="text-pink-400">
        <template #header-actions>
          <div class="relative group w-full sm:w-64">
            ...
            <input 
              v-model="searchedActor" 
              type="text" 
              placeholder="Search specific actor..." 
              class="bg-slate-900/50 border border-slate-700/80 pl-9 pr-4 py-2 rounded-xl text-sm focus:outline-none focus:border-pink-500 text-white w-full shadow-inner transition-colors"
            >
          </div>
        </template>
...
// 🔥 THE MAGIC: Suapkan data yang sudah difilter ke dalam mesin komputasi Technical!
const { genreTrendData, actorQualityData, actorPortfolioData, qualityQuantityData, searchedActor } = useTechnicalData(filteredMoviesByTime)
...
const mixedOptions = {
  ...
  scales: {
    ...
    'y-count': { type: 'linear', position: 'left', title: { display: true, text: 'Total Released', color: '#818cf8' }, ticks: { color: '#818cf8' }, grid: gridLines },
    'y-rating': { type: 'linear', position: 'right', title: { display: true, text: 'Avg Rating', color: '#00f0ff' }, ticks: { color: '#00f0ff' }, grid: { display: false } }
  }
}
</script>